		...
		$.get(
			...
			function(data){	// 미니폼을 보내고 응답이 왔을 때 후속조치
				...
				if(data.resultCode.substr(0,2) == 'S-'){
					$message.empty().append('<div style="color:green; font-size: .9rem;">'+data.msg+'</div>');
					JoinForm__validLoginId = data.loginId;
				}
				else {
					$message.empty().append('<div style="color:red; font-size: .9rem;">'+data.msg+'</div>');
...
<div class="join-form-box con">
	<form action="doJoin" method="POST" class="join-form"
		...
		<div class="form-row">
			...
			<div class="input">
				<input onkeyup="JoinForm__checkLoginIdDup(this)" name="loginId" type="text" placeholder="ID" autocomplete="off"/>
